...
export const getGroupBalances = async (groupId: number): Promise<GroupBalancesPayload> => {
...
export const getGroupBalancesMe = async (
  ...
  const { data } = await api.get<{
    owe_to?: Array<{ user_id: number; amount: number | string }>
    owed_by?: Array<{ user_id: number; amount: number | string }>
